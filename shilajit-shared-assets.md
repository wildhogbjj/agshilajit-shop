# Shared Assets — AG Shilajit / Wild Hog BJJ

Reference file. Both `strategy-gym-first.md` and `strategy-instagram-first.md` pull from this — update here, not in each strategy doc, to avoid drift.

## Product facts
> **ON HOLD (2026-09-05):** the certification and origin claims below — *GMP certified*,
> *Eurofins/third-party tested*, *Made in the UK* — are on hold pending a new certificate
> and have been withheld from new site copy. Do NOT pull them into fresh marketing,
> Instagram captions or gym outreach until the replacement cert is in hand. The 81%
> fulvic acid figure and the lab-cert detail below are from the existing Altai lab report
> and are unaffected.

- Product: AG Shilajit, 30g resin, £30.00 + P&P (standard £5.95 / tracked £6.95)
- Lab cert: 81.0% fulvic acid, 8.14% humic acid, authenticity confirmed, heavy metals within safe limits, no pathogens (Russian/Altai regional lab, product name "Pure Shilajit Resin Extract «Siberian Healer»")
- Magnesium 9,600mg/kg (energy), zinc (male fertility), B12 (rare outside red meat), D3 (fatigue/mood)
- 100% natural, vegan-friendly, gluten-free (GMP certified — ON HOLD, see note above)
- Regional price benchmark: £25–40/30g depending on demographic; £30–35 is the manufacturer's suggested range for a gym demographic like this one

## Purity copy — paragraph version (site homepage hero)
Leads with the 81% fulvic acid stat as the core differentiator vs. UK competitors who don't disclose theirs.

## Purity copy — bullet version (alternative, for social/flyers)
- **81% fulvic acid** — lab-verified, most UK brands don't publish theirs (typically 40–60% where disclosed)
- Sourced from the Altai & Himalayan ranges (Eurofins/third-party tested — ON HOLD, see note above)
- No fillers, no additives — 100% natural, vegan-friendly, gluten-free (GMP certified — ON HOLD)
- 9,600mg/kg magnesium — the noticeable energy lift
- Zinc for male fertility support, B12 (rare outside red meat), D3 for fatigue and mood
- Full certificate of analysis available to download — check it yourself, not just our word

## Tagline
"Pure discipline. Pure ingredients." — jiu jitsu + shilajit wordplay, used on social/partners page, not the shop homepage hero.

## New product line assets (Ashwagandha & Lion's Mane — Coming Soon)
- Pages: `shop.wildhogbjj.co.uk/ashwagandha-shilajit`, `/lions-mane-shilajit` — notify-me capture only, no order form yet
- Source posters (Drive): MARKETING > WEBSITE > A G SHILAJIT > GRAPHICS FOR SHILAJIT SHOP
- Local repo copies: `images/ashwagandha-shilajit-poster.jpg`, `images/lions-mane-shilajit-poster.jpg` (full marketing posters — usable as-is for Instagram posts)
- Product-only crops (no text/background, used in the on-page product box): `images/ashwagandha-shilajit-cutout.jpg`, `images/lions-mane-shilajit-cutout.jpg`

## Contact / links
- WhatsApp: 07956322437
- Shop: agshilajit-shop (GitHub Pages, wildhogbjj/agshilajit-shop repo)
- Instagram: @wildhogbjj audience (not the supplier's account)
- Orders inbox: orders@wildhogbjj.co.uk

## Gym target list (Hertfordshire / North London borders)
| Gym | Town | Angle |
|---|---|---|
| Norton Gym WGC | Welwyn Garden City | BJJ/local collab angle |
| Rise Gym | Stevenage & WGC | Lab-cert-led, no BJJ tie-in |
| NoBull Fitness | Stevenage | Lab-cert-led, no BJJ tie-in |
| New Horizon CrossFit | Hatfield | Recovery/energy angle |
| Gosling Sports Park | Welwyn Garden City | Lower priority, longer cycle |
| Anytime Fitness Borehamwood | Borehamwood | Locally owned & operated (per their own listing) — lab-cert-led |
| Club Moativation | Borehamwood | Independent, worth checking in person before messaging |
| CrossFit Shenley | Shenley | Small independent box (Pursley Farm) — recovery/energy angle fits CrossFit crowd |
| Fernandes Fitness | Shenley | Independent PT business, single-owner — direct lab-cert-led message to the owner |
| Xcelerate Gym Edgware | Edgware | Functional training gym — lab-cert-led, no BJJ tie-in |

Note: most other Edgware "gyms" surfaced in search were chains (PureGym, Anytime Fitness Mill Hill) or low-confidence listicle entries — not included until verified.
